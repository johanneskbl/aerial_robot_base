...
class SendIgmpReportNode(Node):
    ...
    def send_igmp_v2_membership_report(self, group_ip, interface_ip):
        ...
        MAX_RESP_TIME = 0  # Not used in the Membership Report
        CHECKSUM_PLACEHOLDER = 0

        group_bytes = socket.inet_aton(group_ip)

        igmp_packet = struct.pack(
            "!BBH4s", IGMP_MEMBERSHIP_REPORT_TYPE, MAX_RESP_TIME, CHECKSUM_PLACEHOLDER, group_bytes
        )
        ...
        igmp_packet = struct.pack("!BBH4s", IGMP_MEMBERSHIP_REPORT_TYPE, MAX_RESP_TIME, checksum, group_bytes)
        ...
        self.get_logger().info(
            "[{}] IGMPv2 Membership Report sent to {} from {}".format(dt_now, group_ip, interface_ip)
        )
