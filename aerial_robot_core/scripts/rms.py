...
h: Stop the calculation and output the RMS results.
"""
...
class RMSNode(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(PoseControlPid, "debug/pose/pid", self.callback, 10)
...
if __name__ == "__main__":
    ...
    try:
        while rclpy.ok():
            ...
            key = getKey(settings)
            if key == "s":
                node.get_logger().info("[RMS] Start to calculate RMS errors")
                node.start_flag = True
            elif key == "h":
                node.get_logger().info("[RMS] Stop calculation")
                rms = [0.0] * 6
                if node.pose_cnt > 0:
