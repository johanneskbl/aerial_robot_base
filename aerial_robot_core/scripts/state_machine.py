...
class WayPoint(BaseState):
    ...
    def execute(self, userdata):
        ...
        for i, waypoint in enumerate(self.waypoints):
            if len(waypoint) == 3:
                ret = self.robot.goPos(
                    pos=waypoint, pos_thresh=self.pos_thresh, vel_thresh=self.vel_thresh, timeout=self.timeout
                )
            elif len(waypoint) == 4:
                ret = self.robot.goPosYaw(
                    pos=waypoint[:3],
                    yaw=waypoint[-1],
...
class TransformWithPose(BaseState):
    ...
    def execute(self, userdata):
        ...
        for i, target_angles in enumerate(self.target_joint_trajectory):
            ...
            ret = self.robot.poseConvergenceCheck(
                timeout_remaining, target_pos, target_rot, self.pos_thresh, rot_thresh=self.rot_thresh
            )
            if not ret:
                self.robot.get_logger().warning(
